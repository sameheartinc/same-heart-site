...
export default function ThreadDetail({ threadId }: { threadId: string }) {
  ...
  // public thread; only the signed-in-only extras (presence, Skin,
  // Practice tiers, Evolution unlocks) are skipped when there's no
  // user. Reading a PRIVATE community's thread still comes back empty
  // -- that's enforced by RLS on the read itself (see load() below and
  // supabase/schema.sql), not by anything client-side here.
  useEffect(() => {
    (async () => {
  ...
  async function load(uid?: string | null) {
    ...
    setThread(t);
    const r = await listReplies(threadId);
    setReplies(r);
  ...
  async function handleFlag(targetType: ReactionTargetType, targetId: string, category?: string) {
    if (!userId) {
      router.push(`/login?next=${encodeURIComponent(`/commons/t/${threadId}`)}`);
      return;
    }
    if (flaggedMap[targetId]) return;
  ...
  async function handleReply(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await createReply({ threadId: threadId, profileId: userId, body: replyBody });
      setReplyBody("");
      await load();
    } catch {
    ...
    >
      <div style={{ maxWidth: "680px", margin: "0 auto" }}>
        ...
        {/* Rob, Sep 15 2026: "build the share buttons into the threads
            ...
            no community at all) now renders for anyone -- same
            no-login-wall shape as app/commons/exchange/[id]/, gated
            only at the moment someone tries to react, flag, or
            reply. */}
        ...
        {userId ? (
          ...
          // Same "browse freely, sign in the moment you want to do
          // something" shape as the Exchange transmission page's own
          // Resonate button -- see app/commons/exchange/[id]/TransmissionDetail.tsx.
          <div style={{ padding: "16px", borderRadius: "10px", border: "1px dashed var(--border)", textAlign: "center" }}>
            <p style={{ margin: "0 0 10px", fontFamily: "var(--font-body)", fontSize: "0.88rem", color: "var(--ink-dim)" }}>
              Sign in to join the conversation.
            </p>
            <Link
              href={`/login?next=${encodeURIComponent(`/commons/t/${threadId}`)}`}
              style={{ display: "inline-block", padding: "10px 18px", borderRadius: "10px", border: "none", background: ACCENT, color: "#1a0d10", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.82rem", textDecoration: "none" }}
            >
              Sign in
            </Link>
          </div>
